...
Telegram, MAX. Пустые CRM-карточки без `chat_id` в список не попадают.
...
Кнопка создания передаёт карточку контакта в Wazzup через `POST /v3/contacts`,
после чего отправляет сообщение «Здравствуйте!» через `POST /v3/message`.
Отправка первого сообщения создаёт диалог и возвращает настоящий `chatId`.
...
Первое подключение
    ...
    ├── выделение отсутствующих chat_id
    └── помещение найденных записей в очередь ручной проверки

Дальнейшая работа
    ├── message.add → создание только отсутствующего контакта
    ├── существующие имена никогда не меняются автоматически
    ├── неполные данные → очередь «Требуют проверки»
...
`messages_dump` не считается справочником контактов и не используется для
переименования. Поле `user_name` из CSV не переносится в имя контакта. Основным
источником данных нового личного чата служит объект `recipient` вебхука
`message.add`. Если в нём нет `name` и `username`, запись можно проверить и
добавить вручную в интерфейсе.

Вебхуки обрабатываются в фоне. Повторная доставка одного события отсеивается по
`meta.idempotency_key`, поэтому она не создаёт дубликаты.
...
| `GET /api/partner/pending-contacts` | Кандидаты для ручной проверки | Сессия |
| `POST /api/partner/pending-contacts/{id}/approve` | Подтвердить и добавить контакт | Сессия |
| `POST /api/partner/pending-contacts/{id}/dismiss` | Скрыть кандидата | Сессия |
